...
// Charged pushes are serialised: feeds are fetched by concurrent workers, and two batches in flight at once
// would both read the same remaining budget, land in the dataset before either charge is recorded, and
// deliver unbilled items past the cap (the SDK pushes items before it charges for them).
...
async function pushChargedNow(records: ItemRecord[]): Promise<boolean> {
    for (let i = 0; i < records.length; i += PUSH_BATCH_SIZE) {
        ...
        const allowed = isPayPerEvent ? eventsWithinBudget(CHARGE_EVENT, itemsCharged) : wanted.length;
